 ...
 * VALUEIN:默认情况下，是mr框架所读到的一行文本内容，String;此处用Text KEYOUT:是用户自定义逻辑处理完成之后输出数据中的key,在此处是单词，String；此处用Text
 * VALUEOUT，是用户自定义逻辑处理完成之后输出数据中的value，在此处是单词次数，Integer，此处用IntWritable
 *
 * @author ：yrewzjs
 * @date ：Created in 2019/10/13 15:28
 * @modified By：
 */
public class WordcountMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
    ...
    protected void map(LongWritable key, Text value, Context context)
        ...
        String province = words[words.length - 1].substring(0, 3);
        // 3 将单词输出为<单词，1>
